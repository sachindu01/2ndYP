...
const Collection = () => {
  ...
  const sortProduct = () => {
    ...
    switch (sortType){
      case 'low-high':
        setFilterProducts(fpCopy.sort((a,b)=>(a.quantity-b.quantity)));
        break;
      
      case 'high-low':
        setFilterProducts(fpCopy.sort((a,b)=>(b.quantity-a.quantity)));
        break;
  ...
  return (
    <div className='flex flex-col sm:flex-row gap-1 sm:gap-10'>
      {/* Filter Options */}
      <div className='min-w-60'> {/* Corrected min-w class */}
      ...
      <div className='flex-1'>
        <div className='flex justify-between text-base sm:text-2xl mb-4'>
          ...
          {/*Product SOrt */}
          <select onChange={(e)=>setSortType(e.target.value)} className='border-2 border=grat-300 text-sm px-2'>
        ...
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-6'>
          {
            filterProducts.map((item,index)=>(
              <ProductItem key={index} name={item.name} image={item.image} quantity ={item.quantity} id= {item._id} />
            ))
          }
